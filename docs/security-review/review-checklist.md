...
| SR-014 | Medium | PARTIAL | Every external Action reference is pinned to an immutable 40-character commit and CI rejects tag/branch references. Server protected run `33587546197` published verified OCI layouts to the public, repository-linked GHCR package without rebuilding, computed index digest `sha256:37499955cf4b8e18b4f008d1d35425c41d2faeeeb2d3e7b9f8e5ca4c46817f9d` before upload, pulled it back by digest, matched both platform graphs and scanned both served runtimes. The 8 registry evidence subjects passed offline and attestation verification; a clean anonymous pull independently matched the complete graph. Chrome builds a deterministic file-inventoried submission ZIP; Android verifies one certificate DER plus embedded identity. All three release workflows use GitHub-verified `actions/attest v4.2.2`. Active no-bypass main rulesets and protected environments require PR／CI and explicit deployment approval, but the sole administrator still performs both dispatch and approval, so this is not independent release authority. A source-controlled GHCR ledger and internal retention／deletion／ownership-transfer／emergency-revocation policy now preserve exact candidate, platform and decision identities; CI requires two decision actors for approval／retirement, preserved evidence before removal and rejects republishing revoked／retired revisions. There is currently no approved production digest, ordinary deletion is prohibited until an independently durable archive passes retrieval, and owner-level mutation remains technically possible. Durable binary／OCI hosting, archive retrieval evidence, package-owner permission verification, independent policy approval, Chrome Web Store publication, Android distribution evidence, a second release approver and independent review remain open. |
| SR-015 | High | OPEN | The transport path is implemented and carries third-party content only for packages the phone user explicitly selects, through the mandatory per-recipient E2EE and authority-authorized recipient chain. No reviewed release approves it: keep it out of released behavior until the security findings and the two-real-Android OEM/network validation are complete and a reviewed release explicitly changes the gate. |
...
- [ ] **G-01 — Selection-gated fanout.** Inspect release source and artifact to
  prove network fanout admits third-party notification content only for packages
  the phone user has explicitly selected and saved on that device, that an empty
  selection keeps fanout closed, and that no debug flag, remote config, or
  browser-side control can widen that scope. (`EVIDENCE`; SR-015)
...
- [ ] the final decision explicitly authorizes or rejects the implemented
  third-party notification transport for released behavior.

Until then, SevenMirror remains an unreleased Alpha: third-party notification
transport is implemented but unapproved, and the protocol remains provisional.
