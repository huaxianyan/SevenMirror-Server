...
| SR-014 | Medium | PARTIAL | Every external Action reference is pinned to an immutable 40-character commit and CI rejects tag/branch references. Server builds source-bound Linux binaries plus amd64/arm64 OCI layouts from digest-pinned builder/runtime indexes; its offline verifier binds the complete content-addressed graph, runtime identity and per-architecture image-manifest digest. Its protected workflow now defines digest-preserving GHCR publication from those verified layouts, a locally computed multi-architecture index, pull-back by digest, complete graph comparison and registry-served runtime scanning; real default-branch publication evidence remains required. Chrome builds a deterministic file-inventoried submission ZIP; Android verifies one certificate DER plus embedded identity. All three release workflows use GitHub-verified `actions/attest v4.2.2`. Active no-bypass main rulesets and protected environments require PR／CI and explicit deployment approval, but the sole administrator still performs both dispatch and approval, so this is not independent release authority. Production GHCR evidence and policy, durable hosting, Chrome Web Store publication, Android distribution evidence, a second release approver and independent review remain open. |
...
- [ ] **B-06 — Artifact signing/provenance.** Server CI builds source-bound Linux
  ...
  have been verified. The Server workflow now defines digest-preserving GHCR
  publication and pull-side graph/runtime verification; its real execution and
  package policy remain open. Production still requires independent release
  authority, durable hosting, Android distribution identity, and Chrome Web Store
  identity.
  (`PARTIAL`; SR-014)
