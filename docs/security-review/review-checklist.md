...
| SR-008 | Medium | PARTIAL | Chrome inventories raw credentials, non-extractable identity keys, decrypted notification/action state, reply retention, `chrome.storage`, URL/UI surfaces, profile-compromise limits, and deletion gaps. The isolated profile canary covers extension IndexedDB placement, restart, native notifications, diagnostics, `chrome.storage`, interaction URL and closed-profile export. A separate non-headless Cent Browser check now covers the built interaction DOM, exact target/source rendering, reply request binding, immediate input clearing, and URL/diagnostic absence without adding a production debug endpoint. Its Worker message boundary is intercepted, so canonical pending-action persistence remains covered separately rather than claimed by this DOM check. Product retention/clearing, a pinned CI browser artifact, backup/sync, crash/memory, IME and OS-notification-history evidence remain open. |
...
- [ ] **R-11 — Abuse controls.** Reproduce source-address derivation, configurable
  ...
  and slow header/body/WebSocket-auth termination. Then test distributed proxy
  coordination, sustained floods and reconnect storms at deployment capacity.
  (`PARTIAL`; the single-process controls are `EVIDENCE`, remaining scope is SR-010)
...
- [ ] **K-05 — Chrome profile secrets.** `docs/SENSITIVE_DATA.md` inventories raw
  ...
  check verifies target-only DOM rendering, reply binding/input clearing and
  URL/diagnostic absence at an intercepted Worker boundary. Product clearing, a
  pinned CI browser artifact, canonical persistence in the DOM check, backup/sync,
